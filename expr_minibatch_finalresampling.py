...
unbalanced_ratio = 0.05  #unbalance ratio
...
file_name = f'expr_ablation_on_final_resampling_v2_{int(start_time)}'
    ...
    while True:
        try:
            ...
            gen_rkm = RLS_Primal_Gen_RKM_class(f_net, pi_net, 10, img_size, device, 'alexnet',
                                               use_umap=True)
            # train model
            gen_rkm.train_only_minibatch_resampling(ub_MNIST012, num_epochs, batch_size, 1e-4, './SavedModels/',
                          dataset_name='ubMNIST012', save=False)
            x_gen = gen_rkm.random_generation(10000, 3)
            torch.cuda.empty_cache()
            # evaluate
    ...
    while True:
        try:
            ...
            gen_rkm = RLS_Primal_Gen_RKM_class(f_net, pi_net, 10, img_size, device, 'alexnet',
                                               use_umap=True)
            # train model
            gen_rkm.train_only_final_resample(ub_MNIST012, num_epochs, batch_size, 1e-4, './SavedModels/',
                          dataset_name='ubMNIST012', save=False)
            x_gen = gen_rkm.random_generation(10000, 3)
            torch.cuda.empty_cache()
            # evaluate
