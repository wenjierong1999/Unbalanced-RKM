...
class RLS_Primal_Gen_RKM:
    ...
    def __init__(self,
        ...
        self.gaussian_sketching = gaussian_sketching
    ...
    def final_compute(self, dataset : Dataset, batch_size : int):
        ...
        with torch.no_grad():
            ...
            for batch_num in range((N // batch_size) + 1):
                ...
                X_stage1 = dataset.data.to(self.device)[samples_idx_stage1, :, :, :]
                Phi_X_stage1 = self.FeatureMap_Net(X_stage1)
                rls = self.compute_RLS(Phi_X_stage1, guassian_sketching=self.gaussian_sketching, s_d=25)
                if batch_num + 1 == (N // batch_size):
                    samples_idx_stage2 = samples_idx_stage1.to(self.device)[torch.multinomial(rls, (N % batch_size), replacement=True)]
                else:
                    samples_idx_stage2 = samples_idx_stage1.to(self.device)[torch.multinomial(rls, batch_size, replacement=True)]
                sampled_batch_idx.append(samples_idx_stage2)
            sampled_idx = torch.cat(sampled_batch_idx, dim=0)
            x = dataset.data.to(self.device)[sampled_idx, :, :, :]
            Phi_X, U, s = self.primal_KPCA(x)
    ...
    def train(self, dataset : Dataset, epoch_num : int, batch_size : int,
        ...
        for epoch in range(epoch_num):
            ...
            sampled_labels = dataset.target.to(self.device)[torch.cat(sampled_batch_idx, dim = 0)]
            ...
            for batch_idx in sampled_batch_idx:
                imgs = dataset.data.to(self.device)[batch_idx, :, :, :]
...
if __name__ == '__main__':
    ...
    ub_MNIST456 = get_unbalanced_MNIST_dataset('../Data/Data_Store', unbalanced_classes = np.asarray([2]),
    ...
    gen_rkm.train(ub_MNIST456, 20, 328, 1e-4,'../SavedModels/', dataset_name='ubMNIST-demo')
