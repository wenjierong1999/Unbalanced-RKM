...
class Iforest_Primal_Gen_RKM:
    ...
    def compute_iforest_scores(self, Phi_X, n_estimators = 100, contamination = 0.1, use_umap=True, umap_d=25):
        ...
        with torch.no_grad():
            if use_umap:
                ...
                Phi_X = Phi_X.cpu().numpy()
            iforest = IsolationForest(n_estimators=n_estimators,
                                      contamination=contamination)
            iforest.fit(Phi_X)
            iforest_scores = iforest.decision_function(Phi_X)
            ...
            iforest_weights = torch.FloatTensor(iforest_weights).to(self.device)
            #print(iforest_weights.shape)
    ...
    def final_compute(self, dataset: Dataset, batch_size: int, iforest_score):
        '''
        ...
        with torch.no_grad():
            ...
            resampled_idx = torch.multinomial(iforest_score, N, replacement=True)
    ...
    def train(self, dataset: Dataset, epoch_num: int, batch_size: int,
        ...
        iforest_score = self.compute_iforest_scores(Phi_X_rls, use_umap=self.use_umap)
        ...
        for epoch in range(epoch_num):
            ...
            for batch_num in range((N // batch_size) + 1):
                if batch_num + 1 == (N // batch_size):
                    sampled_batch_idx = torch.multinomial(iforest_score, (N % batch_size), replacement=True)
                else:
                    sampled_batch_idx = torch.multinomial(iforest_score, batch_size, replacement=True)
        ...
        #final compute U, h, S
        U, h, s = self.final_compute(dataset, batch_size, iforest_score)
        training_end_time = time.time()
        training_time = round(training_end_time - training_start_time, 1)
        print(f'training time: {training_time}s')
        #save model
        cur_time = int(time.time())
        model_name = f'Iforestclass_PrimalRKM_{dataset_name}_{cur_time}_s{self.h_dim}_b{batch_size}.pth'
        if save:
            torch.save({
...
if __name__ == '__main__':
    ...
    gen_rkm = Iforest_Primal_Gen_RKM(f_net, pi_net, 10, img_size, device, classifier='alexnet', use_umap=True) #resnet18 is preferred umap_d = 25
