...
#add_transforms = transforms.GaussianBlur(kernel_size=21, sigma=(10, 15))

data_train = FastSTL10(root='../Data/Data_Store',split='train', selected_classes=[0,6])
print(data_train.data.shape)
print(torch.unique(data_train.target))
dataloader_train = DataLoader(data_train, batch_size = 100, shuffle=True)
print(torch.unique(dataloader_train.dataset.target))
data_test = FastSTL10(root='../Data/Data_Store',split='test', selected_classes=[0,6])
print(data_test.data.shape)
dataloader_test = DataLoader(data_test, batch_size = 100, shuffle=True)
...
torch.save(resnet18, f'../SavedModels/classifiers/resnet18_06STL10_f{cur_time}_acc{int(acc * 1000)}.pth')